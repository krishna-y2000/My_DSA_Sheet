...
{
    // Checking for descended nodes
	if (node->isLockable == false)
		return;

	narytree* T = node;
	bool flag = false;
    // Checking for ancestors nodes
	while (T != NULL) {
		if (T->isLock == true) {
			flag = true;
			break;
...
int main()
{
	...
	narytree* t5 = new narytree(root->children[0]); // (t1)
// OR	narytree* t5 = new narytree(t1); 	
	...
	cout << "t2 node is locked:"
		...
		<< "\n";
	// Unlocking t4 node.
	unLock(t4);   // Here t4 is already locked 
